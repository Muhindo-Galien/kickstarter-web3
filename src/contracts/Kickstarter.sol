...
/* Errors */
error Kickstarter__TransferFailed();
error Kickstarter__NotOwner();
...
contract Kickstarter {
    ...
    struct projectStruct {
     ...
     modifier ownerOnly(){
        ...
        if (msg.sender != i_owner) revert Kickstarter__NotOwner();
        _;
    }
    ...
    function deleteProject(uint id) public returns (bool) {
        ...
        require(msg.sender == projects[id].owner || msg.sender == i_owner, "Unauthorized Entity");
    ...
    function payTo(address to, uint256 amount) internal {
        (bool success, ) = payable(to).call{value: amount}("");
          if (!success) {
            revert Kickstarter__TransferFailed();
        }
    }
}